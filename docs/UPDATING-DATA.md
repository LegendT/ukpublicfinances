...
**A refresh is done when every figure traces to a named publisher and a named vintage, or the record names the publisher that has not published it and `NEXT-STEPS.md` carries the item.** The tests are a secondary check, not the condition: a figure derived from others satisfies an identity by construction, so `npm test` can pass on a figure nobody published. Rule 8 and the total-revenue item in `NEXT-STEPS.md` are both this problem.

The whole figure set was last re-audited against its primary sources on 21 July 2026; the headline, monthly and indicator figures were refreshed on 22 September 2026, against the ONS August 2026 release that carried the annual data alignment. The pre-2010 historical series remains a best-estimate reconstruction. Each file maps to one official source:
...
A routine month has two deliverables: the newest figures and the month's entry in `monthlyUpdates.json`. The September alignment release has three, because the back series moves too, and reconciling it is the largest of the three rather than a detail of step 4.
...
4. Add the new month's row to `debtTimeseries.json` if you keep the series current. On the September alignment release, also reconcile the recent rows against HF6W, HF6X and J5II rather than only the newest: the alignment revises the back series, and an append-only habit is how the 2024 borrowing row sat £28bn out for a year. Replace a row only where the gap is the size of a revision, a few billion. A gap of tens or hundreds of billions is rule 4 in disguise, a different measure rather than a stale vintage, so leave it and say so in the file's note; that is why the 2000, 2008 and 2010 rows still stand.
...
7. Check the figures no test binds. `assumptions.json` `translator.annualBorrowingGbpBillion` and `indicators.json` `borrowing-gdp` both describe the financial year's borrowing, and nothing fails if they go stale. The refresh of 21 August 2026, against the July release, found the ONS had revised financial year 2025-26 borrowing from £128.0bn to £129.8bn, which moved the simulator baseline and both of these; only the baseline was test-enforced. The alignment of 22 September moved the same figure again, to £134.3bn.
...
9. Re-check the facts no publisher will correct for you, and stamp the date. `primeMinisters.json` is the one that rots: its last entry has an open `to`, and nothing in this repo or in any ONS release can tell you the Prime Minister has changed. Confirm the sitting Prime Minister against the [GOV.UK Prime Minister](https://www.gov.uk/government/ministers/prime-minister) page, which names the incumbent and their start date, and the previous holder's end date against the [past prime ministers](https://www.gov.uk/government/history/past-prime-ministers) page. Correct the list if it has moved, then set `checkedOn` to today. The list was wrong for six weeks in 2026 because nothing prompted anyone to look.
